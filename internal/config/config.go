...
type Config struct {
	...
	ProxyURL             string        `hcl:"proxy_url" env:"PROXY_URL"`
}
...
func Get() Config {
	once.Do(func() {
		log.Printf("[INFO] loading config from: ./config.hcl, ./config.local.hcl, $HOME/.config/AI-RSS-Telegram-Bot/config.hcl")

		loader := aconfig.LoaderFor(&cfg, aconfig.Config{
			EnvPrefix: "NFB",
			Files:     []string{"./config.hcl", "./config.local.hcl", "$HOME/.config/AI-RSS-Telegram-Bot/config.hcl"},
