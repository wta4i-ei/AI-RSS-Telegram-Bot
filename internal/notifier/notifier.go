...
func (n *Notifier) SelectAndSendArticle(ctx context.Context) error {
	topOneArticles, err := n.articles.AllNotPosted(ctx, time.Now().AddDate(-5, 0, 0), 1)
	if err != nil {
		return err
	}

	if len(topOneArticles) == 0 {
		log.Printf("There are no articles available")
		return nil
	}
...
func (n *Notifier) extractSummary(article model.Article) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("не удалось получить содержимое статьи: %s", resp.Status)
	}
	r = resp.Body
...
func (n *Notifier) sendArticle(article model.Article, summary string) error {
	...
	log.Printf("[INFO] The message has been sent")
